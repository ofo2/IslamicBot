...
export default async function handleMyChatMember(ctx, tableManager) {
    ...
    try {
        ...
        if (existingUser.length === 0) {
            ...
            // إرسال رسالة الترحيب فقط إذا كان المستخدم في حالة "انضمام"
            if (status === 'member' || status === 'administrator') {
                const message = ctx.from.username ?
                    `مرحبا @${ctx.from.username}، شكرًا لانضمامك!` :
